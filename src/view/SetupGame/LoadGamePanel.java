...
/**
 * The class Load game panel shows a list of previous games started by the user,
 * so that they can keep the game configuration and start a new game with it.
 ...
 * @version 22/5/2021
 */
public class LoadGamePanel extends JPanel implements StartGame {
    /**
     * The constant BTN_MAPS.
     */
    public static final String BTN_MAPS = "BTN_MAPS_LOAD";
    /**
     * The constant BTN_START.
     */
    public static final String BTN_START = "BTN_START_NEW_LOAD";
    /**
     * The constant BTN_COLOR.
     */
    public static final String BTN_COLOR = "BTN_COLOR_LOAD";
    /**
     * The constant BTN_SETTINGS.
     */
    public static final String BTN_SETTINGS = "BTN_SETTINGS_LOAD";
    /**
     * The constant BTN_BACK.
     */
    public static final String BTN_BACK = "BTN_BACK_LOAD";
    /**
     * The constant COMBO_BOX_GAME.
     */
    public static final String COMBO_BOX_GAME = "COMBO_GAME_LOAD";

    /**
     * The constant SELECT_GAME_LABEL.
     */
    public static final String SELECT_GAME_LABEL = "LOAD GAME:  ";
    /**
     ...
     * The Jcb select game.
     */
    private JComboBox <String> jcbSelectGame;
    /**
     * The Jtf game name.
     */
    ...
    public LoadGamePanel() {
        ...
        // Combo Box Panel to choose
        JPanel jpGameChooser = new JPanel();
        jpGameChooser.setOpaque(false);
        jpGameChooser.setLayout(new BoxLayout(jpGameChooser, BoxLayout.X_AXIS));

        // Select Game Label
        JLabel jlSelectGame = new JLabel();
        jlSelectGame.setText(SELECT_GAME_LABEL);
        jlSelectGame.setFont(ViewUtilities.getBetweenUsFont(FONT_SIZE));
        jlSelectGame.setForeground(Color.white);
        jpGameChooser.add(jlSelectGame);
        // Combo Box
        jcbSelectGame = new JComboBox<>();
        jcbSelectGame.setMaximumSize(new Dimension(320,25));
        jcbSelectGame.setActionCommand(COMBO_BOX_GAME);
        //jcbSelectGame.setBounds(50, 50, 90, 20);
        jpGameChooser.add(jcbSelectGame);
        centerPanel.add(jpGameChooser);
        ...
        //adding a text field
        jtfGameName = new JTextField();
        jtfGameName.setMaximumSize(new Dimension(Integer.MAX_VALUE, jtfGameName.getMinimumSize().height));
        jpGameName.add(jtfGameName);
        jpGameName.add(new Box.Filler(minSize,prefSize,maxSize));
    ...
    public void registerController(ActionListener listener) {
        ...
        jcbSelectGame.addActionListener(listener);
    ...
    public void resetUI() {
        for (int i = 0; i < games.size(); i++) {
            if (games.get(i).getName().equals(jcbSelectGame.getSelectedItem())) {
                jcbCharacters.setSelectedIndex(games.get(i).getNumCharacters() - 4);
                jcbImpostors.setSelectedIndex(games.get(i).getNumImpostors() - 1);
                // resetting previous selected color
                for (int j = 0; j < jbColors.length; j++) {
                    jbColors[j].setBorder(new LineBorder(ViewUtilities.COLORS[j], 30, true));
                }
                selectedColour = games.get(i).getPlayerColourIndex();
                jbColors[selectedColour].setBorder(new CompoundBorder(new LineBorder(Color.white, 2), new LineBorder(ViewUtilities.COLORS[selectedColour], 30, true)));
                jlChosenMap.setText(games.get(i).getMap().getName());
                break;
            }
        }
        revalidate();
    ...
    public Game createNewGame() {
        if (jlChosenMap.getText().isEmpty() || selectedColour == -1 || jtfGameName.getText().isEmpty() || jlChosenMap.getText().equals(MAPS_LABEL_DEMO)) {
            return null;
        }
        String name = jtfGameName.getText();
    ...
    public void setGames(List<Game> games) {
        jcbSelectGame.removeAllItems();
        for (Game game : games) {
            jcbSelectGame.addItem(game.getName());
        }
        this.games = games;
